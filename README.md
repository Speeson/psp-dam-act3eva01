...
|-------|----------------|
| `Cocina` | Gestiona la lista, el lock, y el ciclo de vida de los hilos |
| `Cocinero` | Hilo que toma un pedido, lo "prepara" (sleep) y lo registra |
| `Pedido` | Objeto simple para almacenar datos del pedido |
| `Main` | Punto de entrada. Crea la Cocina y los Pedidos |
...
|-------------|----------------|
| `cocina.py` | Gestiona la lista, el Lock, y el ciclo de vida de los hilos |
| `cocinero.py` | Hilo que toma un pedido, lo "prepara" (sleep) y lo registra |
| `pedido.py` | Objeto simple para almacenar datos del pedido |
| `main.py` | Punto de entrada. Crea la Cocina y los Pedidos |
...
1. **Inicialización**: `main.py` crea una instancia de `Cocina`

2. **Creación de Pedidos**: `main.py` crea entre 6 y 10 `Pedidos` y los añade a la `Cocina`

3. **Inicio Servicio**: Se llama a `cocina.iniciar_servicio(3)`, que crea y lanza 3 hilos `Cocinero`

4. **Competencia**: Los 3 hilos `Cocinero` compiten para adquirir el Lock y entrar al método `tomar_pedido()`
...
6. **Registro**: Al terminar de preparar, el cocinero compite de nuevo por el Lock para entrar a `registrar_pedido()` y escribir en `log_pedidos.txt`

7. **Finalización**: Los hilos terminan su bucle `run()` cuando `tomar_pedido()` devuelve `None`

8. **Join**: El hilo principal (en `cocina.py`) espera (`join()`) a que los 3 hilos `Cocinero` terminen

9. Se imprime "Todos los pedidos han sido procesados."
...
| Característica | Proceso (Usado en ACTEVA02) | Hilo (Usado en ESTA Actividad) |
|----------------|----------------------------|--------------------------------|
| **Memoria** | Espacio de memoria propio y aislado | Comparten el mismo espacio de memoria del proceso padre |
| **Creación** | Más costosa (lenta) | Más ligera (rápida) |
| **Comunicación** | Difícil (Requiere IPC: tuberías, sockets, semáforos...) | Fácil (variables compartidas, locks, monitores...) |
| **Aislamiento** | Alto (un proceso no "rompe" a otro) | Bajo (un hilo puede corromper datos de otro) |
...
- [Java synchronized Methods](https://docs.oracle.com/javase/tutorial/essential/concurrency/syncmeth.html)
